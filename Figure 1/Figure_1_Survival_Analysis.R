...
# Interpretation: 12/35 Datasets violate the proportional hazards assumption
...
# Export Results
write.csv(final_results, "rmst_results DCV.csv", row.names = FALSE)
...
ggplot(final_results, aes(x = RMST_diff, y = miR, color = significance)) +
  ...
  labs(
    title = "",
    x = "Restricted Mean Survival Time Difference (DCV)",
    y = "miRNA mutant",
    color = "Significance"
  ) +
