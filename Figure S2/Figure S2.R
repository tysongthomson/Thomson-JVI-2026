# RScript for Generating the Survival Curves utilised for Figure S2
# This script utilises the FHV folder data and rmst_results FHV.csv provided in Figure 2 folder.
...
# Load RMST results
rmst <- read.csv(file.path("rmst_results FHV.csv"), stringsAsFactors = FALSE)

# Get all CSV files in the data directory
files <- list.files(data_dir, pattern = "\\.csv$", full.names = TRUE)

# Generate Survival Curves (Fig. S2)
for (file in files) {
  ...
  p <- ggsurvplot(
    ...
    break.time.by = 1,
    xlab = "Days",
    ylab = "Proportional Survival (FHV)",
  ...
  out_file <- file.path(
    output_dir,
    paste0(file_base, "_FHV_survival_curve.pdf")
  )
