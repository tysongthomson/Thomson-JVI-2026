# Script for Accumulation Analysis of DCV RNA Results

# Virus: Drosophila C virus (DCV, isolate EB) [RefSeq: NC_001834.1]
# Titre: 10E7 IU/mL
# Fly: w1118 Control vs miRNA Mutant
# Collection Time: 3-dpi
# Amplicon: 203 bp, targeting [RefSeq: NP_044945.1]
# Fig. 5A, Table S4

# Rationale
#  Determine whether miRNA mutant line changes DCV viral RNA accumulation compared to paired control.

# Approach
#  Differences in DCV viral RNA accumulation between miRNA mutant and control were analysed using Mean
...
ggplot(final_results, aes(x = log2FC, y = miRNA, color = signif)) +
  ...
  labs(
    x = "DCV RNA Accumulation (Log2 Fold Change)",
    y = "miRNA mutant",
    color = "Significance"
  ) +
