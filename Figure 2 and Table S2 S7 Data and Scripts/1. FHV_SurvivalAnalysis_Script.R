...
# Interpretation: 9/34 Datasets violate the proportional hazards assumption
...
# Save output table
write.csv(final_results, "Table S2. RMST_FHV.csv", row.names = FALSE)
...
ggplot(final_results, aes(x = RMST_diff, y = miR, color = significance)) +
  ...
  labs(
    title = "",
    x = "Mean Survival Time Difference (FHV)",
    y = "miRNA mutant",
    color = "Significance"
  ) +
